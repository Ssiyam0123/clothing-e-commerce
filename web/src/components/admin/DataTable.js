      ...
      <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800/50">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
          {data?.map((item, rowIdx) => (
            <tr key={item._id || rowIdx} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 transition-colors group">
              {columns.map((col, colIdx) => (
                <td key={colIdx} className="px-8 py-6 whitespace-nowrap text-sm font-bold text-zinc-800 dark:text-zinc-200">
                  {col.render ? (
                    col.render(item)
                  ) : col.key === 'image' ? (
          ...
          {(!data || data.length === 0) && (
            <tr>
              <td colSpan={columns.length + (actions ? 1 : 0)} className="px-8 py-24 text-center">
                ...
                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">No Database Records Found</p>
              </td>
            </tr>
          )}
        </tbody>
