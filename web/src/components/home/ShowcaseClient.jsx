...
export default function ShowcaseClient() {
  ...
  const dynamicCategorySections = useMemo(() => {
    if (!categories || !products || categories.length === 0 || products.length === 0) return [];
    return categories.slice(0, 5).map(cat => {
      const catProducts = products.filter(p => String(p.category?._id || p.category) === String(cat._id)).slice(0, 8); 
      return { id: cat._id, title: cat.name, slug: cat.slug, products: catProducts };
    }).filter(s => s.products.length >= 4); 
  }, [categories, products]);

  if (!isMounted || loading) {
    return <div className="py-32 flex justify-center"><span className="animate-spin h-10 w-10 border-4 border-indigo-600 rounded-full border-t-transparent"></span></div>;
  }

  return (
    <>
      <motion.div variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        <FlashSaleTeaser activeSale={flashSaleProducts?.flashSale} flashSaleProducts={flashSaleProducts} ui={ui} lang={lang} />
      </motion.div>
      ...
      <motion.div variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        <ProductSection title={ui.featTitle} subTitle={ui.featSub} products={featuredProducts} lang={lang} isDarkBg={true} />
      </motion.div>
      
      <motion.div variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
        <ProductSection title={ui.newTitle} subTitle={ui.newSub} products={newArrivals} lang={lang} showLoadMore={true} ui={ui} />
      </motion.div>
      ...
      <div className="py-16 space-y-24 relative">
        ...
        {dynamicCategorySections.map((section, index) => (
          <motion.section 
            ...
            className={`py-12 ${index % 2 !== 0 ? 'bg-gray-50/50 dark:bg-white/5' : ''}`}
          >
            <div className="container mx-auto px-4 md:px-6 mb-8 flex justify-between items-end">
              <div>
                <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-gray-900 dark:text-white uppercase italic">
                  {section.title}
                </h2>
            ...
            >
              {section.products.map((product) => (
                <motion.div key={product._id} variants={itemVariants} className="min-w-[280px] md:min-w-[320px] lg:min-w-[350px]">
