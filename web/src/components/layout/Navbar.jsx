...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1800px] mx-auto px-4 md:px-10 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            {/* Search Trigger */}
            <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="p-2.5 text-zinc-500 hover:text-black dark:hover:text-white transition-colors">
              <Search size={18} />
            </button>
            
            {/* Wishlist Link */}
            <Link href="/wishlist" className="relative p-2.5 text-zinc-500 hover:text-rose-500 transition-colors">
              <Heart size={18} fill={wishlistCount > 0 ? "#e11d48" : "none"} className={wishlistCount > 0 ? 'text-rose-600' : ''} />
              {wishlistCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-rose-600 rounded-full border-2 border-white dark:border-black" />}
            </Link>

            {/* Shopping Bag */}
            <Link href="/cart" className="relative p-2.5 text-zinc-500 hover:text-black dark:hover:text-white transition-colors">
              <ShoppingBag size={18} />
              {cartTotalItems > 0 && (
            ...
            <button onClick={() => setLang(lang === 'en' ? 'bn' : 'en')} className="hidden lg:block text-[9px] font-black uppercase text-zinc-500 hover:text-rose-600 px-2 transition-all">
              {lang === 'en' ? 'BN' : 'EN'}
            </button>

            <button onClick={toggleTheme} className="p-2.5 text-zinc-500 hover:text-black dark:hover:text-white transition-colors hidden sm:block">
            ...
            <div className="relative ml-2" ref={dropdownRef}>
              {isLoading ? (
                <div className="w-10 h-10 rounded-full bg-zinc-100 dark:bg-zinc-900 animate-pulse border border-zinc-200 dark:border-zinc-800" />
              ) : isAuthenticated ? (
                <div 
                ...
                >
                  ...
                  <ChevronDown size={14} className={`text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white transition-transform duration-300 ${isProfileDropdownOpen ? 'rotate-180' : ''}`} />
                    ...
                    {isProfileDropdownOpen && (
                      ...
                      >
                        <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 mb-2">
                          <p className="text-[10px] font-black uppercase tracking-widest text-rose-600 mb-1">{isAdmin ? 'Administrator' : 'Syndicate Member'}</p>
                          <p className="text-sm font-black text-zinc-900 dark:text-white truncate uppercase tracking-tighter">{user.name}</p>
                        </div>
                        
                        <Link href="/profile" className="flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-2xl transition-all">
                          <User size={14} /> Identity Details
                        </Link>
                        
                        {isAdmin && (
                          <Link href="/admin" className="flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-2xl transition-all">
                            <LayoutDashboard size={14} /> Control Center
                          </Link>
                        )}
                        
                        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-2xl transition-all border-t border-zinc-100 dark:border-zinc-800 mt-2">
            ...
            <button className="p-2 lg:hidden dark:text-white ml-2" onClick={() => setIsSidebarOpen(true)}>
              <Menu size={24} />
            </button>
          </div>
        </div>
          ...
          {isSearchOpen && (
            ...
            >
              <form onSubmit={handleSearchSubmit} className="max-w-5xl mx-auto relative">
                ...
                />
                <button type="submit" className="absolute right-6 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-black dark:hover:text-white"><Search size={22} /></button>
              </form>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* 📱 Mobile Sidebar Menu (Synced with authStore) */}
      <AnimatePresence>
        {isSidebarOpen && (
          <div className="fixed inset-0 z-[9999] lg:hidden"> 
            ...
            >
              <div className="flex justify-between items-center mb-16">
                ...
                <button onClick={() => setIsSidebarOpen(false)} className="p-2 bg-zinc-100 dark:bg-white/5 rounded-full dark:text-white">
                  <X size={24} />
                </button>
              </div>
              ...
              <nav className="space-y-8 flex-1">
                {NAV_LINKS.map(link => (
                  <Link 
                    ...
                    className={`block text-5xl font-black uppercase tracking-tighter italic transition-all ${
                      pathname === link.href ? 'text-rose-600 scale-105 ml-2' : 'text-black dark:text-white opacity-40'
                    }`}
                  >
                    {link[lang]}
                  </Link>
                ...
                {isAuthenticated && (
                   <Link href="/profile" onClick={() => setIsSidebarOpen(false)} className="block text-5xl font-black uppercase tracking-tighter italic text-zinc-300 dark:text-zinc-700 hover:text-white transition-colors">
                     PROFILE
                   </Link>
                )}
              </nav>
              ...
              <div className="pt-10 border-t border-zinc-100 dark:border-white/5 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setLang(lang === 'en' ? 'bn' : 'en')} className="py-4 bg-zinc-50 dark:bg-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400">
                    {lang === 'en' ? 'বাংলা' : 'English'}
                  </button>
                  <button onClick={() => toggleTheme()} className="py-4 bg-zinc-50 dark:bg-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400 flex items-center justify-center gap-2">
                    {theme === 'dark' ? <Sun size={14} /> : <Moon size={14} />} {theme}
                  </button>
                </div>
                {isAuthenticated ? (
                  <button onClick={handleLogout} className="w-full py-6 text-rose-600 text-[10px] font-black uppercase tracking-[0.3em] bg-rose-50 dark:bg-rose-950/20 rounded-[1.5rem] shadow-sm">
                    TERMINATE SESSION
                  </button>
                ) : (
                  <Link href="/login" onClick={() => setIsSidebarOpen(false)} className="block w-full py-6 text-center bg-black dark:bg-white text-white dark:text-black text-[10px] font-black uppercase tracking-[0.3em] rounded-[1.5rem] shadow-xl">
                    JOIN THE SYNDICATE
                  </Link>
                )}
              </div>
