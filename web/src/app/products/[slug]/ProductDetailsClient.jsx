...
// Lazy load ReviewSection
const ReviewSection = dynamic(() => import('@/components/store/ReviewSection'), {
...
export default function ProductDetailsClient({ product: initialProduct }) {
  ...
  // Fallback: if product not passed (should not happen), fetch it client-side
  useEffect(() => {
    if (!initialProduct && typeof window !== 'undefined') {
      const slug = window.location.pathname.split('/').pop();
      api.get(`/products/details/${slug}`)
        .then(res => {
          setProduct(res.data);
          setLoading(false);
        })
  ...
  const { data: relatedData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/products?category=${product.category._id}&limit=5`);
      return data;
    },
  ...
  const inWishlist = useMemo(() => 
    wishlistItems?.some(p => String(p._id) === String(product?._id)) || false, 
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex items-center justify-center dark:text-white font-black uppercase tracking-widest text-center px-6 italic opacity-50">
        Protocol Null: Product Not Found
      </div>
    );
  }
          ...
          <div className="lg:col-span-7 flex flex-col items-center">
            <div className="lg:sticky lg:top-28 w-full max-w-[620px]">
              <div className="relative aspect-[3/4] overflow-hidden lg:rounded-[3.5rem] bg-zinc-100 dark:bg-zinc-900 shadow-2xl">
                <Image
                  key={selectedImage}
                  src={getImageUrl(product.images?.[selectedImage], 1000, 85)}
                  alt={product.name}
                  fill
                  priority
            ...
            <section className="space-y-6 mb-12">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`p-3 rounded-full transition-all ${inWishlist ? 'text-rose-500 scale-110' : 'text-zinc-300 dark:text-zinc-700'}`}
