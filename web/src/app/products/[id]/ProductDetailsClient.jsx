...
import OptimizedImage from '@/components/common/OptimizedImage'; // 🌟 Optimized Image
...
const DICTIONARY = {
  ...
  bn: {
    ...
    freeShip: 'ফ্রি শিপিং',
    freeSub: '৫০ ডলারের উপরের অর্ডারে',
...
export default function ProductDetailsClient({ productId, initialProduct }) {
  ...
  // 🌟 SENIOR FIX: React Query now uses initialData from the Server Component
  const { data: product, isLoading, error, refetch: refetchProduct } = useQuery({
    ...
    initialData: initialProduct, // 👈 Zero layout shift!
  });

  const { data: reviewsData, refetch: refetchReviews } = useQuery({
    queryKey: ['reviews', productId],
    queryFn: async () => (await api.get(`/reviews/product/${productId}`)).data,
    enabled: !!productId,
  });
  ...
  useEffect(() => {
    if (productsData?.products) {
      setRelatedProducts(productsData.products.filter(p => p._id !== productId).slice(0, 4));
    }
  ...
  const handleAddToCart = async () => {
    ...
    if (!user) {
      if (confirm('Please login to add items to your cart')) router.push(`/login?redirect=/products/${productId}`);
      return;
    }
    setIsAdding(true);
    try {
  ...
  const handleBuyNow = () => {
    ...
    if (user) {
      ...
      router.push('/login?redirect=/checkout?type=direct');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] dark:bg-[#050505] font-sans pb-20 transition-colors duration-700 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10 relative z-10">
        ...
        >
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 p-6 sm:p-10 lg:p-16">
            ...
            <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-6">
              <motion.div variants={fadeUp} className="relative aspect-[3/4] bg-zinc-100 dark:bg-zinc-900 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden group border border-zinc-200/50 dark:border-white/5 pointer-events-none">
                {product.images && product.images[selectedImage] ? (
                  <OptimizedImage
                    src={product.images[selectedImage]}
                    alt={product.name}
                    sizes="(max-width: 768px) 100vw, 50vw"
                    className="w-full h-full grayscale-[10%] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000 ease-in-out"
                  />
                ) : (
                ...
                {product.discount > 0 && (
                  <div className="absolute top-4 left-4 sm:top-6 sm:left-6 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white px-4 py-2 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-lg border border-zinc-200 dark:border-zinc-800 z-10">
                    -{product.discount}%
                  </div>
                )}
                {product.isFeatured && (
                  <div className="absolute top-4 right-4 sm:top-6 sm:right-6 bg-black/50 backdrop-blur-md text-white px-4 py-2 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-lg z-10">
                    Featured
                  </div>
                )}
              </motion.div>
              ...
              {product.images && product.images.length > 1 && (
                <motion.div variants={fadeUp} className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 no-scrollbar">
                  {product.images.map((img, idx) => (
                    ...
                    >
                      <OptimizedImage src={img} alt={`${product.name} ${idx + 1}`} sizes="100px" />
                    </motion.button>
                  ))}
                </motion.div>
              )}
        ...
        >
          <ReviewSection productId={productId} onReviewChange={() => { refetchProduct(); refetchReviews(); }} />
        </motion.div>
