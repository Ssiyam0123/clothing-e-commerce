...
const PremiumStatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-4 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${styles[status]}`}>
      {text[status]}
    </span>
  );
};
...
export default function AdminFlashSales() {
  ...
  const handleToggleActive = async (saleId, currentActive) => {
    try {
      ...
      swalToast(`Campaign ${!currentActive ? 'Activated' : 'Deactivated'}`, 'success');
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    const confirmed = await swalConfirm('Delete Campaign?', 'This promotion will be permanently removed from all linked products.');
    if (confirmed) {
      try {
  ...
  return (
    <div className="space-y-10 pb-20 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase mb-2">Campaigns</h1>
          <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em]">Flash Sale & Promotions</p>
        </div>
        <Link 
      ...
      {allLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 p-8 h-[400px] animate-pulse"></div>
          ))}
        </div>
        ...
        <div className="col-span-full text-center py-24 bg-white dark:bg-[#0a0a0a] rounded-[3rem] border border-dashed border-zinc-200 dark:border-zinc-800 shadow-inner">
           ...
           <h2 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-tighter mb-2">No Active Campaigns</h2>
           <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em]">Launch a new protocol to start selling</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {allFlashSales?.map((sale) => {
            ...
            return (
              <div key={sale._id} className="bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm hover:-translate-y-1 transition-all duration-500 flex flex-col p-8 group">
                <div className="flex justify-between items-start mb-8">
                  <div className="bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 px-4 py-2 rounded-2xl flex flex-col items-center">
                    <span className="text-3xl font-black leading-none">{sale.discount}%</span>
                    <span className="text-[9px] font-black uppercase tracking-widest mt-1">Off</span>
                  </div>
                  <div className="flex flex-col items-end gap-3">
                    ...
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${
                        sale.isActive ? 'bg-zinc-900 dark:bg-white text-white dark:text-black border-transparent' : 'bg-transparent text-zinc-500 border-zinc-200 dark:border-zinc-700 hover:border-zinc-400'
                      }`}
                    >
                ...
                <h3 className="text-2xl font-black text-zinc-900 dark:text-white uppercase tracking-tighter line-clamp-1 mb-6">{sale.name}</h3>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex justify-between items-center p-3 rounded-xl bg-zinc-50 dark:bg-[#111] border border-zinc-100 dark:border-zinc-800/50">
                    <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Starts</span>
                    <span className="text-xs font-bold text-zinc-900 dark:text-white uppercase">{start.toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 rounded-xl bg-zinc-50 dark:bg-[#111] border border-zinc-100 dark:border-zinc-800/50">
                    <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Ends</span>
                    <span className="text-xs font-bold text-zinc-900 dark:text-white uppercase">{end.toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 rounded-xl bg-zinc-50 dark:bg-[#111] border border-zinc-100 dark:border-zinc-800/50">
                    <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Inventory</span>
                    <span className="text-xs font-black text-zinc-900 dark:text-white">{sale.products?.length || 0} Items</span>
                  </div>
                </div>
