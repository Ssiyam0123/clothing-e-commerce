...
export default function ProductManageNavigation({ activeTab, product }) {
  ...
  return (
    <div className="fixed bottom-6 left-4 right-4 z-50">
      <TabsPrimitive.List className="max-w-2xl mx-auto bg-background/80 backdrop-blur-2xl border border-border/10 p-1.5 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center justify-between relative group w-full h-auto gap-1">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <TabsPrimitive.Trigger
              ...
              value={tab.id}
              className={cn(
                "relative flex flex-col items-center gap-1 py-2 px-1 rounded-2xl transition-all duration-500 min-w-[60px] md:min-w-[75px] flex-1 text-center border-0 bg-transparent shadow-none hover:text-foreground cursor-pointer select-none outline-none",
