...
function AdminProductsContent() {
  ...
  const handleDelete = async (id) => {
    const confirmed = await swalConfirm('Purge Product?', 'This action is permanent.');
    if (!confirmed) return;
  ...
  const handleToggleActive = async (id, currentStatus) => {
    try {
      ...
      const message = err.response?.data?.message || err.message || 'Could not sync with databanks.';
  ...
  const columns = [
    ...
    },
    {
      label: 'Price',
      render: (item) => (
        <span className="font-black text-zinc-900 dark:text-white">৳{item.price.toFixed(2)}</span>
      ),
    },
    {
      label: 'Inventory',
      render: (item) => {
        ...
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[10px] font-bold text-zinc-500">{stock} Units</span>
          </div>
        );
      },
    },
...
export default function AdminProductsPage() {
  return (
    <Suspense fallback={<div className="p-20"><Loader /></div>}>
