  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 space-y-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
        ...
        <button onClick={() => router.back()} className="text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">
          ← Cancel & Return
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 p-8 md:p-12 shadow-sm space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3">Category Name *</label>
            <input 
              type="text" 
        ...
        <div>
          <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-4">Cover Visual Asset</label>
          <div className="flex flex-col sm:flex-row items-center gap-8">
            <div className="relative w-full sm:w-1/2 border-2 border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-[#111] rounded-[2rem] p-10 text-center hover:border-zinc-900 dark:hover:border-white transition-all cursor-pointer">
              <input type="file" accept="image/*" {...register('imageFile')} onChange={handleImageChange} className="absolute inset-0 opacity-0 cursor-pointer" />
              <span className="text-4xl block mb-3 grayscale opacity-50">🖼️</span>
              <span className="text-[10px] font-black text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">Select Image</span>
            </div>
            ...
            {imagePreview && (
              <div className="w-full sm:w-1/2 relative h-48 rounded-[2rem] overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <img src={imagePreview} alt="Preview" className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-10 border-t border-zinc-100 dark:border-zinc-800">
          ...
          >
            {isSubmitting ? 'Processing...' : (isEdit ? 'Sync Architecture' : 'Initialize Category')}
          </button>
          <button 
            type="button" 
