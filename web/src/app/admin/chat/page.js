...
import { Send, User, MessageCircle, Clock, Check, CheckCheck, Shield } from 'lucide-react';
...
export default function AdminChat() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('connect', () => console.log('🛡️ Command Center: Link Established'));
  ...
  const fetchConversations = async () => {
    try {
      ...
      console.error('📡 Intelligence Failure: Could not sync conversations', err);
    }
  };
  ...
  const selectConversation = async (conv) => {
    ...
    try {
      ...
      console.error('📡 Intelligence Failure: Could not retrieve message logs', err);
    } finally {
  ...
  const handleSendMessage = () => {
    ...
    const customer = selectedConv.participants.find(p => p.role === 'customer');
    if (!customer) return;
      ...
      <div className="w-80 border-r border-zinc-200 dark:border-white/5 flex flex-col bg-zinc-50/50 dark:bg-[#080808] backdrop-blur-3xl">
        <div className="p-8 border-b border-zinc-200 dark:border-white/5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400">Comms Hub</h2>
          </div>
          <p className="text-xl font-black uppercase italic tracking-tighter">Frequencies</p>
        </div>
          ...
          {conversations.length > 0 ? (
            conversations.map((conv) => {
              ...
              return (
                ...
                >
                  <div className={`h-12 w-12 rounded-full flex items-center justify-center overflow-hidden border-2 transition-colors ${
                    isActive ? 'border-white/20' : 'border-zinc-200 dark:border-white/5 group-hover:border-zinc-300'
                  }`}>
                    {customer?.avatar ? (
                      <img src={customer.avatar} alt="" className="h-full w-full object-cover" />
                    ) : (
                      <User size={20} className={isActive ? 'text-zinc-400' : 'text-zinc-500'} />
                    )}
                  </div>
                  <div className="flex-1 text-left min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <p className="text-[11px] font-black uppercase tracking-wider truncate">{customer?.name || 'Unknown'}</p>
                      {conv.unreadCount > 0 && (
                    ...
                    <p className={`text-[10px] font-medium truncate opacity-60 ${isActive ? 'text-zinc-300' : 'text-zinc-500'}`}>
            ...
            <div className="p-8 text-center opacity-30">
              <p className="text-[10px] font-black uppercase tracking-widest italic">No Active Frequencies</p>
            </div>
          )}
        </div>
      </div>
        ...
        {selectedConv ? (
              ...
              <div className="flex items-center gap-5">
                <div className="relative">
                  <div className="h-12 w-12 rounded-full bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center overflow-hidden border border-zinc-200 dark:border-white/5 shadow-inner">
                    {selectedConv.participants.find(p => p.role === 'customer')?.avatar ? (
                      <img src={selectedConv.participants.find(p => p.role === 'customer').avatar} alt="" className="h-full w-full object-cover" />
                    ) : (
                ...
                <div>
                  <h3 className="text-[13px] font-black uppercase tracking-[0.2em]">{selectedConv.participants.find(p => p.role === 'customer')?.name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] font-black text-green-500 uppercase tracking-widest italic">Live Transmission Active</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-4">
                 <div className="px-4 py-2 bg-zinc-100 dark:bg-zinc-900 rounded-full border border-zinc-200 dark:border-white/5">
                   <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500">ID: {selectedConv._id.slice(-8)}</span>
                 </div>
              </div>
            </div>
                    ...
                    >
                      <div className={`flex gap-4 max-w-[70%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          <div className={`p-5 rounded-[2rem] text-[11px] font-bold leading-relaxed shadow-sm transition-all hover:shadow-xl ${
                            isMe 
                          ...
                          <div className={`flex items-center gap-3 mt-3 px-1 ${isMe ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-400 space-y-8 p-12">
            ...
            <div className="text-center max-w-sm">
              <p className="text-[14px] font-black uppercase tracking-[0.5em] mb-4 text-zinc-900 dark:text-zinc-100">Frequency Idle</p>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] leading-relaxed opacity-40">
                Satellite link established. Waiting for frequency selection to begin tactical communication protocols.
              </p>
            </div>
          </div>
        )}
