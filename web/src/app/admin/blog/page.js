...
import { 
  ...
  Filter, MoreHorizontal, BookOpen, Clock, 
...
export default function AdminBlogDashboard() {
  ...
  const handleDelete = async (id) => {
    const confirmed = await swalConfirm("Purge Narrative?", "This sequence will be permanently removed from the foundry archives.");
    if (confirmed) {
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-rose-600 transition-colors" size={16} />
            <input 
              type="text" 
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard icon={<BookOpen size={18}/>} label="Published" value={blogs?.filter(b => b.status === 'PUBLISHED').length} color="text-emerald-500" />
        <StatsCard icon={<Clock size={18}/>} label="Drafts" value={blogs?.filter(b => b.status === 'DRAFT').length} color="text-amber-500" />
        <StatsCard icon={<BarChart3 size={18}/>} label="Total Views" value={blogs?.reduce((acc, b) => acc + (b.viewCount || 0), 0)} color="text-indigo-500" />
        <StatsCard icon={<Filter size={18}/>} label="Categories" value={new Set(blogs?.map(b => b.category)).size} color="text-rose-500" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredBlogs?.length > 0 ? (
              ...
              <div className="flex-1 text-center md:text-left space-y-1">
                <div className="flex flex-wrap justify-center md:justify-start items-center gap-3">
                  ...
                  <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${
                    post.status === 'PUBLISHED' ? 'text-emerald-500 border-emerald-500/20 bg-emerald-500/5' : 'text-zinc-400 border-zinc-400/20 bg-zinc-400/5'
                  }`}>
                    {post.status}
                  </span>
                </div>
          ...
          <div className="py-20 text-center bg-zinc-50 dark:bg-zinc-900/20 rounded-[3rem] border border-dashed border-zinc-200 dark:border-zinc-800">
            <p className="text-xs font-black uppercase tracking-[0.4em] text-zinc-400">Zero Narrative Sequences Found</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="bg-white dark:bg-zinc-900/40 p-6 rounded-[2rem] border border-zinc-100 dark:border-white/5 shadow-sm">
      <div className="flex items-center gap-4">
        <div className={`p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800 ${color}`}>{icon}</div>
        <div>
          <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400">{label}</p>
          <h4 className="text-2xl font-black tracking-tighter dark:text-white mt-0.5">{value}</h4>
        </div>
      </div>
    </div>
  );
